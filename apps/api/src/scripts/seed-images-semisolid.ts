/**
 * seed-images-semisolid.ts
 * ─────────────────────────────────────────────────────────────────────────────
 * Bulk-uploads semi-solid recipe hero images from content/semi-solid-images/
 ...
 *
 * Run:
 *   npm run seed:images:semisolid
 *
 * Flags (set as env vars before running):
 *   SKIP_SLUGS=m-01        Comma-separated slugs to skip (already uploaded).
 ...
 *
 * Notes:
 *   Files must be named m-01.jpg, m-02.jpg, … (case-insensitive).
 *   Files with non-matching names (e.g. image3.jpg) are skipped automatically.
 */
...
async function run() {
  ...
  // Separate matched vs unrecognised
  ...
  if (skippedUnrecognised.length) {
    console.log(`  ⚠  Unrecognised filenames (skipped — rename to m-XX.ext to include):`);
