...
interface Rule { _id: string; ingredient: string; match: string[]; effects: Effect[]; enabled: boolean }
interface Condition { code: string; label: string }
interface Ingredient { name: string; count: number }
interface RecipeLite { ingredients?: { nameEn?: string }[] }
...
// Curated starter rules — condition codes are filtered to what's actually in the
// vocabulary before saving, so a missing condition just drops that effect.
...
export function DietRulesPage() {
  ...
  useEffect(() => {
    ...
    api<Record<string, { label: string; order?: number }>>('/api/admin/health-flags')
      .then(m => setConditions(Object.entries(m).sort((a, b) => (a[1].order ?? 0) - (b[1].order ?? 0)).map(([code, s]) => ({ code, label: s.label }))))
      .catch(() => {});
  ...
  const countFor = (match: string[]) => {
    ...
    return recipes.filter(r => (r.ingredients ?? []).some(i => kws.some(k => norm(i.nameEn ?? '').includes(k)))).length;
  };
  ...
  async function saveDraft() {
    ...
    try {
      const body = { ingredient: draft.ingredient.trim(), match: draft.match, effects: draft.effects, enabled: true };
  ...
  return (
    <div className="p-5 md:p-7">
        ...
        <div className="bg-bone border border-ink/[0.14] rounded-[14px] p-5 mb-5">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => runApply(true)} disabled={busy}
              className="bg-brand text-white rounded-[9px] px-4 py-2 text-[12.5px] font-semibold disabled:opacity-50">Apply {apply.added + apply.changed + apply.removed} changes</button>
      ...
      <div className="overflow-x-auto rounded-[14px] border border-ink/[0.11]">
        <table className="w-full min-w-[680px] border-collapse">
          ...
          <tbody>
              ...
              <tr className="border-t border-ink/[0.08]">
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap gap-1.5">
                    {r.effects.length === 0 ? <span className="text-ink/35 text-[12px]">none</span> : r.effects.map((e, i) => (
                ...
                <td className="px-4 py-3 text-right text-[12.5px] text-ink/55 tabular-nums whitespace-nowrap">{countFor(r.match)} recipes</td>
              ...
              {draft?._id === r._id && (
                <tr>
                  <td colSpan={5} className="p-0 bg-brand-bg/40 border-t border-brand/30">
                    <RuleEditor
                      draft={draft} conditions={conditions} ingredients={ingredients} countFor={countFor} ruled={ruledNames}
                      onChange={setDraft} onSave={saveDraft} onCancel={() => { setDraft(null); setMsg(null); }} busy={busy} inline
                    />
                  </td>
                </tr>
              )}
            ...
            {rules.length === 0 && (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-[13px] text-ink/45">No rules yet — “Suggest starter rules” to begin, or add one below.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {!draft && (
        <button type="button" onClick={() => setDraft({ ingredient: '', match: [], effects: [], enabled: true })}
      ...
      {/* Add mode shows the editor below the table; Edit mode expands inline under its row. */}
      {draft && !draft._id && (
        <RuleEditor
          draft={draft} conditions={conditions} ingredients={ingredients} countFor={countFor} ruled={ruledNames}
          onChange={setDraft} onSave={saveDraft} onCancel={() => { setDraft(null); setMsg(null); }} busy={busy}
        />
      )}
    </div>
  );
...
interface Draft { _id?: string; ingredient: string; match: string[]; effects: Effect[]; enabled: boolean }

// Searchable dropdown of the ingredients that exist across the app's recipes
// (live from /api/admin/diet-rules/ingredients, so new recipe ingredients appear
// automatically). Ingredients that already have a rule are marked ✓ ruled.
...
function RuleEditor({ draft, conditions, ingredients, countFor, ruled, onChange, onSave, onCancel, busy, inline }: {
  draft: Draft; conditions: Condition[]; ingredients: Ingredient[];
  countFor: (m: string[]) => number; ruled: Set<string>;
      ...
      <div className="grid sm:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 items-center">
            ...
            <input value={kw} onChange={e => setKw(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addKw(); } }}
